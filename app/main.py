...
from .models import Expense, Attachment, RecurringTransaction, LinkedAccount, TransactionSplit
...
async def upload_statement(file: UploadFile = File(...), session: Session = Depends(get_session)):
    ...
    if filename.lower().endswith('.csv') or file.content_type in ('text/csv', 'application/vnd.ms-excel'):
        ...
        raise HTTPException(status_code=400, detail='Only CSV statement uploads are supported')
